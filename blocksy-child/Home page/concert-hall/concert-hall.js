...
function initRenderer() {
    ...
    renderer.shadowMap.autoUpdate = false; // manually trigger once after scene is built
...
async function enterConcertHall() {
    ...
    // Force a high-quality render before starting the camera zoom
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    renderer.shadowMap.needsUpdate = true;
    renderer.render(scene, camera);

    // Smooth camera transition to orbit (2s from intro)
    // IMPORTANT: NO heavy loading during zoom — keep render loop 100% smooth
    setView('orbit');
    bindInputListeners();

    // Phase 2 assets (HDR, Tone.js, tablet, envMap) are already loaded during bootScene.
    // The only work left is: crossfade lights, init audio, add bloom.

    // Start crossfade immediately — lights ramp during zoom for a seamless reveal
    crossfadeToConcertHallLights();

    // Init audio in parallel (non-blocking, user can see scene while samples load)
    initAudio();
    initMIDI();

    // Add bloom after a brief delay to avoid frame drop during zoom start
    setTimeout(() => {
        if (!phase2Loaded) {
            initPostProcessing();
            renderer.shadowMap.needsUpdate = true;
...
function scrollToExplore() {
    // ── SCROLL OFFSET ──────────────────────────────────────────────────────
    // Positive value = stop further UP (shows more of the section header).
    // Negative value = scroll further DOWN (hides more of the section header).
    // Default 0 → top of #pm-studio-banner aligns with top of viewport.
    // ───────────────────────────────────────────────────────────────────────
    const SCROLL_OFFSET = -190; // px — negative = scroll further down past studio banner

    const target = document.getElementById('pm-studio-banner')
                || document.getElementById('pm-explore-section');
    if (target) {
...
function openCurtain() {
    ...
    if (!curtain) return;
    // Minimal delay — one rAF to let the first render paint before curtain slides
    requestAnimationFrame(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => { curtain.style.display = 'none'; }, 380);
        }, 1650);
    });
}
...
async function bootScene() {
    ...
    // Start Phase 2 network fetches NOW — overlaps with piano model download.
    // HDR + Tone.js load in parallel with the piano GLB, so by the time the user
    // clicks "Play Concert Hall", everything is already cached.
    preloadPhase2Assets();

    // Load the piano model (the hero asset)
    await loadPiano();

    // Build the concert hall geometry immediately (stage, floor, walls) — lightweight
    // but do NOT add the full lighting yet (lights come with the button)
    buildConcertHallGeometry();

    // Pre-create the 3D tablet during boot so it's instantly visible on enter.
    // The tablet is a lightweight canvas texture — no audio, no bloom, just geometry.
    await preloadPhase2Assets(); // ensure envMap is ready for tablet frame material
    ...
    // Start intro camera drift toward orbit
    cameraTarget.position.copy(CAMERA_PRESETS.orbit.position);
    cameraTarget.lookAt.copy(CAMERA_PRESETS.orbit.target);
    cameraTarget.fov = CAMERA_PRESETS.orbit.fov;

    sceneReady = true;
    setState('landing');
    openCurtain(); // reveal piano by sliding the velvet curtain open
    log('Phase 1 complete. Scene + tablet ready, awaiting Concert Hall entry.');
