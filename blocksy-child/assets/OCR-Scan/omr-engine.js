...
console.log('[PianoModeOMR] Engine v5.0 loaded — all modules ready');
    ...
    /**
     * Detect beams connecting stemmed notes.
     * Audiveris BeamsBuilder approach: look for horizontal thick bands between stem endpoints.
     */
    detectBeams: function(bin, width, height, noteheads, staffSpacing) {
        var sp = staffSpacing;
        var beamHeight = Math.max(3, Math.round(sp * 0.45));
        var beamTolerance = Math.round(sp * 0.8);

        // Group noteheads by staff
        var staffGroups = {};
        var n, head;
        ...
        // For each staff group, check pairs of stemmed notes for beams
        for (var sKey in staffGroups) {
            ...
            for (var i = 0; i < group.length - 1; i++) {
                ...
                // Both must have stems in the same direction
                if (!left.hasStem || !right.hasStem) continue;
                if (left.stemDir !== right.stemDir) continue;

                // Distance between them should be reasonable (within ~3 staff spacings)
                var xDist = right.centerX - left.centerX;
                if (xDist < sp * 0.5 || xDist > sp * 3.5) continue;

                // Check for beam: horizontal band connecting stem endpoints
                var stemY1 = left.stemEndY;
                var stemY2 = right.stemEndY;
                var beamY = Math.round((stemY1 + stemY2) / 2);
                var beamLeft = left.centerX;
                var beamRight = right.centerX;

                // Count beams by scanning vertically around the stem endpoint region
                var nBeams = 0;
                ...
                for (var by = scanTop; by <= scanBot; by++) {
                    // Check if this row has a horizontal black band between the two notes
                    var samplePoints = 5;
                    var blackSamples = 0;
                    for (var si = 0; si <= samplePoints; si++) {
                        var sampleX = Math.round(beamLeft + (beamRight - beamLeft) * si / samplePoints);
                ...
                // Subtract 1 if we counted the staff line
                // Beams should be no more than 3
                nBeams = Math.min(3, nBeams);

                if (nBeams > 0) {
                    // Apply beam count to both notes
    ...
    /**
     * Classify duration of each notehead based on fill, stems, beams, flags.
     */
    classifyDuration: function(noteheads) {
        for (var n = 0; n < noteheads.length; n++) {
            var head = noteheads[n];
            var beats = 4; // default: whole note
            ...
            if (!head.hasStem && !head.isFilled) {
                ...
                var divisions = 0;
                if (head.beamCount > 0) {
                    divisions = head.beamCount;
                } else if (head.flagCount > 0) {
                    divisions = head.flagCount;
                }
                ...
                // Filled but no stem — treat as quarter
                beats = 1;
                durationType = 'quarter';
            }
    ...
    /**
     * Assign pitch to each notehead based on position relative to staff lines.
     */
    assignPitch: function(noteheads, staves) {
        // Pitch mapping: position index 0 = top line, increases going down
        // Treble clef: line 0 = F5, space 0-1 = E5, line 1 = D5, etc.
        // Bass clef: line 0 = A3, space 0-1 = G3, line 1 = F3, etc.

        var treblePitches = [
            // posIndex: -6=C6(84), -5=B5(83), -4=A5(81), -3=G5(79), -2=F5(77)
            // -1=E5(76), 0=F5(77)... 
            // Let's use a proper mapping. posIndex 0 = first line (top).
            // Going from top: F5, E5, D5, C5, B4, A4, G4, F4, E4, D4, C4...
            // posIndex: 0=F5, 0.5=E5, 1=D5, 1.5=C5, 2=B4, 2.5=A4, 3=G4, 3.5=F4, 4=E4
        ];

        // MIDI note for each half-position from the top line
        // Treble clef top line = F5 = MIDI 77
        var trebleBase = 77; // F5 at position 0 (line index 0)
        // Each position step down = one diatonic step down
        // F5, E5, D5, C5, B4, A4, G4, F4, E4, D4, C4, B3...
        var trebleDiatonic = [77, 76, 74, 72, 71, 69, 67, 65, 64, 62, 60, 59, 57, 55, 53, 52, 50, 48, 47, 45];
        // posIndex 0 = line 0 = F5(77), posIndex 1 = line 1, etc.
        // posIndex i corresponds to half-steps of the scale:
        // We map using diatonic scale. posIndex in half-position units (0, 0.5, 1, 1.5, ...)
        ...
        // Map for MusicXML: diatonic scale step + octave
        var trebleSteps = [
        ...
        for (var n = 0; n < noteheads.length; n++) {
            ...
            // Calculate position index: 0 = top line, 0.5 = space between line 0 and 1
            ...
            var minPos = -6;
            var maxPos = 14;
            if (posIndex < minPos) posIndex = minPos;
            if (posIndex > maxPos) posIndex = maxPos;
            ...
            // Adjust posIndex to array index (can go negative for ledger lines above)
            var arrIdx = posIndex + 4; // offset: -4 maps to index 0 for 2 ledger lines above
            // Actually, let's re-do this properly:
            // posIndex 0 = line index 0 (top line). Our diatonic array index 0 = top line.
            // Ledger lines above: posIndex -1 = space above line 0, posIndex -2 = first ledger line above
            var lookupIdx = posIndex + 2; // give room for ledger lines above (posIndex -2 = index 0)
