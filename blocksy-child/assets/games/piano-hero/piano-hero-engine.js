...
class VirtualPianoVisualizer {
    ...
    async initMIDI() {
        ...
        try {
            ...
            console.log('MIDI: Access granted (USB + Bluetooth)');
            ...
            console.log('MIDI: Access denied or error -', error.message);
        }
    }
    ...
    open() {
        ...
        console.log('✓ Piano Hero opened in ' + (standaloneMode ? 'standalone' : 'integrated') + ' mode');
    }
    ...
    processMidi(notes, name) {
        ...
        if (handMethod === 'range') {
            ...
            for (const durThreshold of [1.5, 1.3, 1.8, 2.5]) {
                ...
                if (longNotes.length >= notes.length * 0.03 && shortNotes.length >= notes.length * 0.1) {
                    ...
                    if (shortMedianPitch - longMedianPitch >= 2) {
                        ...
                        console.log(`Hand detection: duration-based (threshold=${durThreshold}x, long=${longMedianPitch}, short=${shortMedianPitch})`);
                        for (const n of notes) {
            ...
            if (!durationMethodWorks) {
                ...
                if (hasBassPattern || hasWideSpreads) {
                    ...
                    console.log(`Hand detection: beat-position (${singleLowCount} bass, ${lowNoteInGroupCount} wide groups, split=${splitPoint})`);
                    for (const n of notes) {
            ...
            if (!durationMethodWorks) {
                ...
                if (groupsWithSplit > groups.length * 0.2 && groupsWithSplit >= 3) {
                    ...
                    console.log(`Hand detection: positional (${groupsWithSplit}/${groups.length} groups, split=${globalSplit})`);
                    for (const group of groups) {
                        if (group.length >= 2) {
            ...
            if (handMethod === 'range') {
                ...
                console.log(`Hand detection range fallback: split at MIDI ${bestGapMidi}`);
                for (const n of notes) {
        ...
        console.log(`Hand detection: method=${handMethod}, rightGroup=${rightGroup}, leftGroup=${leftGroup}, groupKey=${groupKey}`);
