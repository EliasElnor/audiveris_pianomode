...
if(!this.onG)this.state=this.vy<0?'jump':(this.gliding?'glide':'fall');
...
dissolve(){this.state='dissolving';this.dissolveT=0;}
...
const col=cn.validated?'rgba(80,200,120,.9)':'rgba(180,220,255,.9)';
const glowCol=cn.validated?'rgba(80,200,120,.6)':'rgba(140,180,255,.6)';
// Note head
ctx.save();ctx.shadowColor=glowCol;ctx.shadowBlur=cn.validated?18:12;
...
ctx.font='bold 14px Montserrat,sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
...
ctx.fillText('Use keys or piano',sx,midY-4);
ctx.shadowBlur=0;}
ctx.restore();}}
...
this.player.x=br.x-this.player.w-2;this.player.vx=0;
if(this.state==='playing')this._activeBarrier=cb;
break;}}
