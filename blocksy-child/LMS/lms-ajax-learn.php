...
function pm_load_level_sections() {
    ...
    if (!empty($modules) && !is_wp_error($modules)) {
        foreach ($modules as $mod) {
            $mq = new WP_Query(['post_type'=>'pm_lesson','tax_query'=>['relation'=>'AND',['taxonomy'=>'pm_module','field'=>'term_id','terms'=>$mod->term_id],['taxonomy'=>'pm_level','field'=>'slug','terms'=>$level]],'posts_per_page'=>-1,'fields'=>'ids']);
            if ($mq->found_posts > 0) {
                $md = count(array_intersect($mq->posts, $completed));
                ...
                $locked_class = $is_locked ? ' pm-mod-locked' : '';

                if ($is_locked) {
                    // Locked module: no link, blur info, show lock badge
                    $modules_html .= '<div class="pm-mod-row' . $locked_class . '" style="--mc:' . $color . '">';

                    // Lock icon instead of number
                    $modules_html .= '<div class="pm-mod-num" style="background:rgba(100,100,100,0.15);border-color:#333;">';
                    $modules_html .= '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#666" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';
                    $modules_html .= '</div>';
                    ...
                    // Lock badge
                    $badge_class = $lock_type === 'paid' ? 'pm-lock-badge-paid-front' : 'pm-lock-badge-account-front';
                    $badge_icon = $lock_type === 'paid'
                        ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>'
                        : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';
                    $badge_text = $lock_type === 'paid' ? 'Subscribe & Learn' : 'Create an Account';
                    $modules_html .= '<div class="pm-mod-lock-badge ' . $badge_class . '">' . $badge_icon . ' ' . $badge_text . '</div>';
...
function pm_load_article_content() {
    ...
    // Strip scripts and iframes for security
