...
class PianoMode_SEO_Master {
    ...
    private function __construct() {
        ...
        // template_redirect fires BEFORE wp_head — the only safe place to remove
        // wp_robots (hooked to wp_head priority 1) before it outputs its incomplete tag
        add_action('template_redirect', [$this, 'remove_wp_defaults_early']);
    ...
    /**
     * Fired at template_redirect (before wp_head) to remove WP/Blocksy defaults
     * that would otherwise produce duplicate or incomplete tags on custom pages.
     *
     * wp_robots() is hooked to wp_head priority 1.
     * It outputs only: max-image-preview:large
     * Our output_robots_meta() outputs the full: index, follow, max-snippet:-1, ...
     * → removing wp_robots here prevents the duplicate.
     */
    public function remove_wp_defaults_early() {
...
/**
    ...
    private function is_custom_seo_page() {
        ...
        // Game pages (matched by URL against Play admin games)
    ...
    public function output_robots_meta() {
        ...
        // Score taxonomies (composer, style, level) and tags are noindex,nofollow
        // They display raw card layouts on non-formatted pages — block crawling entirely
        if (is_tax('score_composer') || is_tax('score_style') || is_tax('score_level')) {
    ...
    private function get_category_image($category_id) {
        ...
        if (!empty($posts)) {
            return get_the_post_thumbnail_url($posts[0]->ID, 'full');
        }
        
        return home_url('/wp-content/uploads/default-category.jpg');
    }
    ...
    private function get_taxonomy_image($term) {
        ...
        if (!empty($scores)) {
            return get_the_post_thumbnail_url($scores[0]->ID, 'full');
        }
        
        return home_url('/wp-content/uploads/default-score.jpg');
    }
...
/**
 * Robots meta pour les pages de tags et auteurs
 * noindex,nofollow — ces pages affichent des cards sur des pages non formatées PianoMode
 */
