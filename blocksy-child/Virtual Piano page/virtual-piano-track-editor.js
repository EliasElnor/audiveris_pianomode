...
class TrackEditor {
    ...
    createUploadTrackButton() {
        ...
        uploadBtn.innerHTML = `
            ...
            <span>Upload Audio File <small style="opacity:0.7;font-size:9px;display:block;">(MP3, WAV, FLAC)</small></span>
        `;
    ...
    addEditButtonToTrack(trackElement) {
        ...
        editBtn.innerHTML = '<span class="edit-btn-icon">✏️</span><span class="edit-btn-text">Edit</span>';
        editBtn.style.display = isMasterTrack ? 'inline-flex' : 'none'; // Hidden by default for non-master
    ...
    openMasterTrackEditor() {
        // Check for recordings: multi-clip system (globalDAW) or legacy single clip element
        const hasMultiClips = window.globalDAW && window.globalDAW.masterRecordingClips && window.globalDAW.masterRecordingClips.length > 0;
        const masterClip = document.getElementById('masterRecordingClip');
        const hasLegacyClip = masterClip && masterClip.style.display !== 'none';
        const hasRecording = hasMultiClips || hasLegacyClip;

        // Remove any existing modal
        document.getElementById('masterEditModal')?.remove();

        // Compute duration text for the modal
        let masterDurationText = document.getElementById('masterClipDuration')?.textContent || '0:00';
        if (hasMultiClips) {
            let maxEnd = 0;
            window.globalDAW.masterRecordingClips.forEach(clip => {
                const clipEnd = (clip.startTime || 0) + clip.duration;
                if (clipEnd > maxEnd) maxEnd = clipEnd;
            });
            const mins = Math.floor(maxEnd / 60);
            const secs = Math.floor(maxEnd % 60);
            masterDurationText = `${mins}:${String(secs).padStart(2, '0')}`;
        }
            ...
            <div class="track-edit-modal-content" style="max-width:500px;">
                ...
                <div class="track-edit-body">
                    ${hasRecording ? `
                        <div class="track-info-row">
                            ...
                            <span id="masterEditDuration" style="color:#D7BF81;">${masterDurationText}</span>
                        </div>
                ...
                <div class="track-edit-footer">
                    ${hasRecording ? `
                        ...
                        <button class="edit-btn primary" id="applyMasterTrimBtn">Apply Cut</button>
                        <button class="edit-btn primary" id="exportMasterBtn">Export Audio</button>
                    ` : `
    ...
    setupMasterEditor(modal) {
        ...
        // Get master duration and blob
        let totalDuration = 0;
        let masterBlob = null;
        if (window.globalDAW && window.globalDAW.masterRecordingClips && window.globalDAW.masterRecordingClips.length > 0) {
            window.globalDAW.masterRecordingClips.forEach(clip => {
                const clipEnd = (clip.startTime || 0) + clip.duration;
                if (clipEnd > totalDuration) totalDuration = clipEnd;
                if (clip.blob && !masterBlob) masterBlob = clip.blob;
            });
        } else {
            // Legacy: try to get duration from clip element
            const durationText = document.getElementById('masterClipDuration')?.textContent || '0:00';
            const parts = durationText.split(':');
            totalDuration = parseInt(parts[0]) * 60 + parseInt(parts[1] || 0);
            // Legacy: try masterRecordingData
            if (window.globalDAW && window.globalDAW.masterRecordingData) {
                masterBlob = window.globalDAW.masterRecordingData;
            }
        }

        // Always try to decode audio blob for accurate waveform visualization
        if (masterBlob && destCanvas) {
            this.drawWaveformFromBlob(masterBlob, destCanvas, totalDuration);
        } else if (destCanvas) {
            // Fallback: try copying from an existing clip canvas
            let srcCanvas = document.getElementById('masterClipCanvas');
            if ((!srcCanvas || srcCanvas.style.display === 'none') && window.globalDAW && window.globalDAW.masterRecordingClips) {
                for (const clip of window.globalDAW.masterRecordingClips) {
                    const clipCanvas = document.getElementById(`canvas-${clip.id}`);
                    if (clipCanvas) { srcCanvas = clipCanvas; break; }
                }
            }
            if (srcCanvas) {
            ...
            // Draw time ruler
            if (totalDuration > 0) {
                this.drawTimeRuler(destCanvas, totalDuration);
            }
        }
        ...
        this._masterTotalDuration = totalDuration;
        this._masterBlob = masterBlob;
        ...
        const updateTrimVisuals = () => {
            ...
            document.getElementById('masterTrimStartVal').textContent = this.formatTimePrecise(startSec);
            document.getElementById('masterTrimEndVal').textContent = this.formatTimePrecise(endSec);

            // Update duration display to show selected range
            const selDuration = endSec - startSec;
            const durEl = document.getElementById('masterEditDuration');
            if (durEl) {
                durEl.textContent = `${this.formatTime(Math.floor(endSec - startSec))} (${startSec.toFixed(1)}s - ${endSec.toFixed(1)}s)`;
            }
        };
        ...
        // Initialize
        document.getElementById('masterTrimEndVal').textContent = this.formatTimePrecise(totalDuration);
        updateTrimVisuals();
        ...
        document.getElementById('previewMasterBtn')?.addEventListener('click', async () => {
            const startSec = (trimStart / 100) * totalDuration;
            const endSec = (trimEnd / 100) * totalDuration;
            await this.previewMasterSelection(startSec, endSec);
        });

        document.getElementById('stopMasterBtn')?.addEventListener('click', () => {
            this.stopPreview();
        });

        // Apply cut button
        document.getElementById('applyMasterTrimBtn')?.addEventListener('click', async () => {
            if (window.globalDAW && window.globalDAW.trimMasterRecording) {
                const btn = document.getElementById('applyMasterTrimBtn');
                if (btn) { btn.textContent = 'Cutting...'; btn.disabled = true; }
                try {
                    await window.globalDAW.trimMasterRecording(trimStart / 100, trimEnd / 100);
                    alert('✅ Cut applied successfully!');
                    modal.remove();
                } catch(err) {
                    alert('❌ Error applying cut: ' + err.message);
                    if (btn) { btn.textContent = 'Apply Cut'; btn.disabled = false; }
                }
            } else {
                alert('❌ Cut function not available. Please record audio first.');
            }
        });
    ...
    // Draw waveform from an audio Blob
    async drawWaveformFromBlob(blob, canvas, totalDuration) {
        try {
            ...
            this.drawTimeRuler(canvas, totalDuration);
        } catch (err) {
            console.warn('Could not decode master audio for waveform:', err);
        }
    }

    // Draw time ruler markers on waveform canvas
    drawTimeRuler(canvas, totalDuration) {
        ...
        else if (totalDuration > 10) interval = 5;
        else interval = 1;
    ...
    // Preview master selection (decoded from blob)
    async previewMasterSelection(startSec, endSec) {
        this.stopPreview();

        // Find the master recording blob
        let blob = this._masterBlob;
        if (!blob && window.globalDAW && window.globalDAW.masterRecordingClips) {
            for (const clip of window.globalDAW.masterRecordingClips) {
                if (clip.blob) { blob = clip.blob; break; }
            }
        }
        if (!blob) return;
        ...
        try {
            ...
            const previewBtn = document.getElementById('previewMasterBtn');
            const stopBtn = document.getElementById('stopMasterBtn');
            if (previewBtn) { previewBtn.classList.add('playing'); previewBtn.innerHTML = '⏸ Playing'; }
            ...
            source.onended = () => {
                if (this.isPlaying) {
                    this.isPlaying = false;
                    this.previewSource = null;
                    const btn = document.getElementById('previewMasterBtn');
                    const sBtn = document.getElementById('stopMasterBtn');
                    if (btn) { btn.classList.remove('playing'); btn.innerHTML = '▶ Preview Selection'; }
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .track-edit-btn-inline .edit-btn-icon {
                font-size: 12px;
                line-height: 1;
                display: inline-flex;
                align-items: center;
            }
            .track-edit-btn-inline .edit-btn-text {
                font-size: 11px;
                font-weight: 600;
    ...
    drawWaveform(audioBuffer, targetCanvas) {
        const canvas = targetCanvas || document.getElementById('waveformCanvas');
        if (!canvas) return;
        ...
        gradient.addColorStop(0, 'rgba(215,191,129,0.8)');
        gradient.addColorStop(0.5, 'rgba(215,191,129,0.4)');
    ...
    sendToMix() {
        ...
        // Register source and auto-assign to an empty track (or create one)
        window.globalDAW.registerAndAssign(track.id, track.name, 'uploaded', {
            buffer: track.buffer,
            startTime: track.startTime,
            endTime: track.endTime,
            volume: track.volume,
        ...
        alert(`"${track.name}" added to track!`);
        this.closeEditModal();
    }
