...
class VirtualPianoRecorder {
                ...
                <div class="recorder-downloads" id="recDownloads" style="display: none;">
                    ...
                    <div class="download-buttons">
                        ...
                        <button class="download-btn download-midi" id="recDownloadMidi">
                            ...
                            MIDI (Piano + Drums)
                            <small style="display:block;font-size:8px;opacity:0.6;font-weight:400;line-height:1.2;">Notes only — no effects, voice or audio tracks</small>
                        </button>
                    </div>
    ...
    async startRecording() {
        ...
        try {
            ...
            // Store reference for cleanup on stop
            this._toneStreamDest = toneStreamDest;
    ...
    stopRecording() {
        ...
        // Disconnect Tone.js master from the recording stream destination
        // This prevents leaked connections that accumulate across recording sessions
        if (this._toneStreamDest) {
            try {
                Tone.getDestination().disconnect(this._toneStreamDest);
            } catch(e) {}
            this._toneStreamDest = null;
        }
    ...
    stopAllAudio() {
        ...
        // Stop global DAW playback
        if (window.globalDAW && typeof window.globalDAW.stop === 'function') {
            window.globalDAW.stop();
        }

        // Stop BackTracks Player
        if (window.backTracksPlayer && typeof window.backTracksPlayer.stop === 'function') {
            window.backTracksPlayer.stop();
        }

        // Stop any audio cut editor preview
        if (window._editorPreviewAudio) {
            try { window._editorPreviewAudio.pause(); } catch(e) {}
            window._editorPreviewAudio = null;
        }
    ...
    async downloadWAV() {
        ...
        try {
            ...
            // Fallback: download as webm with user notification
            alert('⚠️ WAV conversion failed. Downloading as WebM format instead.\n\nThe audio quality is the same, but some DAWs may not import WebM files directly.');
