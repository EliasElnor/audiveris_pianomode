    ...
    Tone.start().then(async () => {
        try {
            ...
            if (!window._masterLimiterInstalled) {
                const masterGainReduction = new Tone.Gain(0.75);
...
class VirtualStudioPro {
    ...
    async initToneJS() {
        ...
        try {
            ...
            // PIANO - Salamander Grand Piano - DIRECT TO DESTINATION
            // Extended sample points for better quality across all octaves
            ...
            const loadPianoSampler = () => {
                this.synths.piano = new Tone.Sampler({
                    ...
                    onerror: (err) => {
                        console.warn('⚠️ Piano sample load error:', err);
                        // Retry up to 3 times with smaller sample set
                ...
                this.synths.piano.toDestination();
            };
            ...
            this.synths.dawPiano = new Tone.Sampler({
                urls: {
                    ...
                    "D#4": "Ds4.mp3", "F#4": "Fs4.mp3", A4: "A4.mp3",
                    C5: "C5.mp3", A5: "A5.mp3", C6: "C6.mp3", A6: "A6.mp3",
                    C7: "C7.mp3", A7: "A7.mp3",
                },
                release: 1,
                baseUrl: "https://tonejs.github.io/audio/salamander/",
            ...
            // Do NOT call toDestination() - will be routed per-track through effect chains
    ...
    playPianoNote(note) {
        ...
        if (this.activeNotes.size >= 16) {
            const oldest = this.activeNotes.keys().next().value;
            this.releaseNote(oldest);
        }
        try {
            prewarmAudioOnce();
    ...
    playTrack(trackId) {
        ...
        track.notes.forEach((noteData, noteIndex) => {
            const timeoutId = setTimeout(() => {
                ...
                // Get synth directly - no playPianoNote indirection
                const synth = vs.synths?.[trackInstrument] || vs.synths?.piano;
                if (!synth) return;

                try {
                    // Use triggerAttackRelease for clean independent playback
                    // This avoids activeNotes conflicts with live playing
