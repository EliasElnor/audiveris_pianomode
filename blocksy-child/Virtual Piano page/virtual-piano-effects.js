...
class VirtualPianoEffects {
    constructor(audioContext) {
        ...
        // Connect internal effects chain: channel → delay → reverb
        // Final output routing (reverb → masterCompressor) is handled by
        // reconnectToEffectsChain() which inserts this chain into the master bus
        this.effectsChain.connect(this.delay);
        this.delay.connect(this.reverb);

        // Route effects output through master bus if available, otherwise direct to destination
        // This will be updated by prewarmAudioOnce() or studio-daw.js when master bus is ready
        this._currentOutput = window._masterCompressor || Tone.getDestination();
    ...
    // Update reverb output to route through a new destination (e.g., master compressor)
    updateOutputRouting(newOutput) {
        if (!newOutput) return;
        try {
