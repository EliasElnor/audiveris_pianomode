document.addEventListener('DOMContentLoaded', function() {
    ...
    class BackTracksPlayer {
            ...
            try {
                if (typeof Tone !== 'undefined' && Tone.context && Tone.context.state === 'running') {
                    ...
                    if (rawCtx && rawCtx.createMediaElementSource) {
                        ...
                        const toneGainNode = Tone.context.createGain();
                        this.mediaSource.connect(toneGainNode);
                        if (window._masterCompressor) {
                            toneGainNode.connect(window._masterCompressor.input);
                        } else if (window.effectsModule && window.effectsModule.effectsChain) {
                            toneGainNode.connect(window.effectsModule.effectsChain.input);
                        } else {
                            toneGainNode.connect(Tone.getDestination().input);
                        }
                ...
                console.warn('Backtrack Tone.js connection failed, using direct playback:', err);
            }
            return false;
        }
        ...
        async togglePlay() {
            ...
            if (this.isPlaying) {
                ...
                this.connectToToneJS();
                const globalVol = (window.globalDAW && window.globalDAW.globalMasterVolume != null)
                    ? window.globalDAW.globalMasterVolume / 100 : 1;
                if (this.btGainNode) {
                    this.btGainNode.gain.value = globalVol;
                } else {
                    this.audio.volume = globalVol;
                }
                try {
                    ...
                    console.warn('Backtrack play error:', playErr);
                }
            }
            this.updatePlayButton();
        }
        ...
        getBlobDuration(blob) {
            return new Promise((resolve) => {
                ...
                tempAudio.addEventListener('loadedmetadata', () => {
                    ...
                    resolve(dur && isFinite(dur) ? dur : 60);
                });
    ...
    class GlobalDAWManager {
        constructor() {
            ...
            this.globalMasterVolume = 100; // Master Vol slider (affects global output)
        ...
        playDrumSoundWithTrackEffects(instrument, track, hitVolume = 1) {
            // Use the SAME drum synths as the live drum machine for identical sound.
            // This routes through the drum machine's limiter -> effects chain -> Tone.js destination,
            // which is the exact same path used during recording.
            // We call playSynthDrum/playUploadedSample directly (no recording hooks).
            ...
            // Check for uploaded samples first (same logic as drum machine)
            if (studio.uploadedSamples && studio.uploadedSamples.has(instrument)) {
                studio.playUploadedSample(instrument, effectiveVol);
                return;
            }

            // Check for custom samples from assets
            if (studio.customSamples && studio.customSamples.has(instrument)) {
                ...
                if (player && player.buffer && studio.isInitialized) {
                    try {
                        ...
                        newPlayer.volume.value = Tone.gainToDb(effectiveVol * studio.masterVolume);
                        newPlayer.start();
                        setTimeout(() => { try { newPlayer.dispose(); } catch(e) {} }, 2000);
                    } catch(e) {}
                    return;
                }
            }

            // Use the LIVE drum machine synths (same ones used during recording)
            if (studio.isInitialized && studio.drums && studio.drums[instrument]) {
        ...
        playPianoNoteWithTrackEffects(note, duration, track, instrument = 'piano') {
            ...
            try {
                // For piano: use a TEMPORARY per-note Sampler to avoid shared-state bugs
                // The old approach connected/disconnected the shared dawPiano per-note,
                // which caused signal doubling on chords and cut-offs on disconnect.
                const dawPiano = window.virtualStudio?.synths?.dawPiano;
                if (instrument === 'piano' && dawPiano && window.virtualStudio?.dawPianoLoaded) {
                    // Ensure persistent per-track gain node for dawPiano
                    // This avoids creating a new Gain for every single note
                    if (!track._dawPianoGain) {
                        track._dawPianoGain = new Tone.Gain(0.8).connect(track.tonePanner);
                        track._dawPianoConnected = false;
                    }

                    // Connect dawPiano to this track's gain ONCE (persistent connection)
                    if (!track._dawPianoConnected) {
                        try {
                            dawPiano.connect(track._dawPianoGain);
                            track._dawPianoConnected = true;
                        } catch(e) {
                    ...
                    // Just trigger the note - routing is already set up
                    dawPiano.triggerAttackRelease(note, noteDuration);
                    return;
                }

                // For other instruments: create temporary isolated synths
                // Volumes reduced to match live playback levels
                let synth;
                switch (instrument) {
                    case 'electric-piano':
                    ...
                    default:
                        synth = new Tone.PolySynth(Tone.Synth, {
                            ...
                            envelope: { attack: 0.005, decay: 0.4, sustain: 0.15, release: 0.3 },
                            volume: -14
                        });
                        break;
                }
                ...
                const disposeDelay = Math.max(2000, duration + 1000);
                setTimeout(() => { try { synth.dispose(); } catch(e) {} }, disposeDelay);
            } catch (error) {
                window.virtualStudio?.playPianoNote(note);
                setTimeout(() => window.virtualStudio?.stopPianoNote(note), duration);
            }
