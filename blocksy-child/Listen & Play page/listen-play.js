...
(function () {
    ...
    let elSearchSection, elHero, elPagination;
    ...
    /* ══════════════════════════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════════════════════════ */
    function init() {
        elSearch        = $('#pm-lp-search');
        elGrid          = $('#pm-lp-grid');
        elLoader        = $('#pm-lp-loader');
        elCount         = $('#pm-lp-count');
        elClear         = $('#pm-lp-clear');
        elSearchSection = $('#pm-lp-search-section');
        ...
        if (elPagination) {
            elPagination.addEventListener('click', (e) => {
                ...
                if (page && page >= 1 && page <= totalPages) {
                    performSearch(page);
                    scrollTo(elSearchSection, -HEADER_OFFSET);
                }
            });
        }
    }
    ...
    /* ══════════════════════════════════════════════════════════════════
       STICKY SEARCH — always sticky, title hidden when stuck
       ══════════════════════════════════════════════════════════════════ */
    function initStickySearch() {
        if (!elSearchSection || !('IntersectionObserver' in window)) return;

        // Sentinel before search section
        const sentinel = document.createElement('div');
        sentinel.style.height = '1px';
        sentinel.style.pointerEvents = 'none';
        elSearchSection.parentNode.insertBefore(sentinel, elSearchSection);

        const obs = new IntersectionObserver(([entry]) => {
            elSearchSection.classList.toggle('is-stuck', !entry.isIntersecting);
        }, { threshold: 0 });
