...
function pianomode_export_admin_page() {
    ...
    <div class="pm-export-wrap">
        ...
        <div class="pm-info-box">
            ...
            métadonnées SEO (meta title, description, mot-clé, URL canonique), URL de l'article, image à la une et URLs
            des images du contenu. Pour les <strong>Scores</strong> : URLs de la partition PDF, partition XML et vidéo YouTube sont également incluses.</p>
        </div>
    </div>
...
function pianomode_handle_export_list() {
    ...
    if ($export_type === 'post' || $export_type === 'both') {
    ...
    if ($export_type === 'score' || $export_type === 'both') {
    ...
    if ($export_format === 'csv') {
        ...
        // En-têtes
        fputcsv($output, array('Type', 'ID', 'Titre', 'URL'), ';');

        // Données
        foreach ($data as $row) {
            fputcsv($output, array($row['type'], $row['id'], $row['title'], $row['url']), ';');
        }
