...
/* ── SEO: Canonical, Robots, Open Graph, JSON-LD for Learn Page ── */
add_action('wp_head', function () {
    $canonical   = home_url('/learn/');
    $title       = 'Learn Piano Online - Free Structured Courses for All Levels | PianoMode';
...
// Access check: user must be logged in AND have completed assessment to see full content
$can_access_content = $logged && $assessment_done;
...
// Modules for active level
function pm_get_modules_for_level($level_slug, $completed_lessons) {
    $modules = get_terms(['taxonomy'=>'pm_module','hide_empty'=>false,'orderby'=>'name']);
    $result = [];
...
<link rel="stylesheet" href="<?php echo get_stylesheet_directory_uri(); ?>/Learn page/hero-learn.css?v=<?php echo filemtime(get_stylesheet_directory() . '/Learn page/hero-learn.css'); ?>">
...
<div class="pm-learn-v4" id="pmLearnV4">
    ...
    <div class="pm-topbar" id="pmTopbar">
        <div class="pm-topbar-inner">
            <div class="pm-tb-group">
                <div class="pm-tb-item pm-tb-hearts">
                    <?php for ($i = 0; $i < 5; $i++) : ?>
                        <svg class="pm-tb-heart-icon <?php echo $i >= $user_hearts ? 'empty' : ''; ?>" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="<?php echo $i < $user_hearts ? '#D7BF81' : '#333'; ?>" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                    <?php endfor; ?>
                </div>
    ...
    <section class="pm-piano-sec" id="pmPianoSec">
        ...
        <div class="pm-piano-stage" id="pmPianoStage">
            ...
            <div class="pm-piano-body-top"><span class="pm-piano-brand">PianoMode</span></div>
    ...
    <section class="pm-modules-sec" id="pmModulesSec">
        ...
        <div class="pm-modules-list" id="pmModulesList">
            <?php if (!empty($active_modules)) : foreach ($active_modules as $mi => $am) :
                ...
                $mod_gated = ($mi > 0); // Only first module (index 0) is accessible
            ?>
            <?php if ($mod_gated) : ?>
            <div class="pm-mod-row pm-mod-row-gated" style="--mc:<?php echo $clr; ?>">
    ...
    <!-- ========== LESSONS ========== -->
    <section class="pm-lessons-sec" id="pmLessonsSec">
        <div class="pm-sec-header pm-sec-header-left">
            <h2>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#D7BF81" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                Available Lessons
            </h2>
            <p>Click a lesson to start learning</p>
        </div>
        <div class="pm-lessons-grid" id="pmLessonsGrid">
            <?php $prev_done = true;
            if ($lessons_query->have_posts()) : while ($lessons_query->have_posts()) : $lessons_query->the_post();
                $lid = get_the_ID();
            ...
            ?>
            <a href="<?php echo $href; ?>" class="pm-lcard pm-lcard-<?php echo $state; ?>" style="--lc:<?php echo $levels[$active]['color']; ?>">
                ...
                <div class="pm-lcard-body">
                    ...
                    <div class="pm-lcard-meta">
                        ...
                        <span>
                            ...
                            <?php echo intval($dur); ?>m
                        </span>
                        <?php endif; ?>
                        <span>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                            <?php echo intval($xp); ?> XP
                        </span>
                        <?php if ($quiz) : ?>
                        <span class="pm-lcard-quiz">
            ...
            <?php $prev_done = $is_done; endwhile; wp_reset_postdata(); else : ?>
            <div class="pm-empty-state full">
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#D7BF81" stroke-width="1.5"><rect x="2" y="6" width="20" height="15" rx="2"/><path d="M8 21V11M12 21V11M16 21V11"/></svg>
...
<script src="<?php echo get_stylesheet_directory_uri(); ?>/Learn page/learn-page.js?v=<?php echo time(); ?>"></script>
<?php get_footer(); ?>