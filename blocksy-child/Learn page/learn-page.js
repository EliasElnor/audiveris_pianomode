...
(function() {
    ...
    // Note label geolocation (Latin vs International)
    // Latin countries: France, Spain, Italy, Portugal, Romania, Belgium,
    // Latin America, French-speaking Africa, etc.
    var LATIN_COUNTRIES = [
    ...
    var useLatinLabels = LATIN_LOCALES.indexOf(userLang) !== -1;
    var LATIN_MAP = { 'C': 'Do', 'D': 'Ré', 'E': 'Mi', 'F': 'Fa', 'G': 'Sol', 'A': 'La', 'B': 'Si' };

    // Try server-side geolocation for accurate country detection
    function detectCountryAndUpdateLabels() {
        ...
        fetch(ajaxUrl, { method: 'POST', body: fd })
            ...
            .then(function(res) {
                if (res.success && res.data && res.data.country_code) {
                    var country = res.data.country_code;
                    var isLatin = LATIN_COUNTRIES.indexOf(country) !== -1;
    ...
    // Update piano key note labels based on locale
    function updateKeyLabels() {
        var noteEls = document.querySelectorAll('.pm-pkey-note');
        noteEls.forEach(function(el) {
            var letter = el.getAttribute('data-note') || el.textContent.trim();
            if (useLatinLabels && LATIN_MAP[letter]) {
                el.textContent = LATIN_MAP[letter];
            } else if (!useLatinLabels && letter.length > 1) {
                // Reverse lookup: find the letter key
                for (var k in LATIN_MAP) {
                    if (LATIN_MAP[k] === letter) {
                        el.textContent = k;
                        break;
                    }
                }
    ...
    // Apply localized note labels (browser-based first, then refine with server geoloc)
    updateKeyLabels();
    detectCountryAndUpdateLabels();
