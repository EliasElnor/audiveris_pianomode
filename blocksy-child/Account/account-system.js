...
(function($) {
    ...
    $(document).ready(function() {
        console.log('🎹 PianoMode Account System v9.0 - Initializing...');
        
        initializeSystem();
        initEventListeners();
        
        console.log('✅ PianoMode Account System loaded successfully');
    });
    ...
    function trackReading(postId) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    console.log('✅ Reading tracked:', postId);
                }
            },
            error: function() {
                console.log('❌ Failed to track reading');
            }
        });
    }
    ...
    function trackDownload(scoreId) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    console.log('✅ Download tracked:', scoreId);
                }
            },
            error: function() {
    ...
    window.pmSaveSightReadingStats = function(stats) {
        if (!pmAccountData.user_logged_in) {
            console.log('⚠️ User not logged in - stats not saved');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    console.log('✅ Sight reading stats saved:', response.data);
                    
                    if (response.data.xp_earned > 0) {
                        showNotification(`+${response.data.xp_earned} XP earned!`, 'success');
                    }
                } else {
                    console.log('❌ Failed to save stats:', response.data);
                }
            },
            error: function() {
                console.log('❌ Connection error while saving stats');
            }
        });
    };
...
console.log('✅ PianoMode Account System JavaScript v9.0 loaded');
